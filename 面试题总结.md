...
cookie：存在客户端，后端在response header里传一个set-cookie字段，存起来，下一次请求客户端会自动带上cookie
...
回流就是改变了dom的结构和位置，需要重新计算，重新生成dom tree或者render tree
重绘就是浏览器不需重新计算元素的几何属性、直接为该元素绘制新的样式
